...
// NOTE: kept aligned to THIS branch's frozen pool (reconcile). main's BAD_STYLE is stricter (adds
// french/italian/persian/brass/… ) but the pool here still carries those curated styles, so the
// narrower reconcile regex is authoritative for the audit gate. simplifyMedium/MED_FAMILY below ARE main's.
