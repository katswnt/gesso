...
import {
  ...
  buildConfirmationInput, buildLocalizationInput, resolveConfirmedLocalization, spatialRowsForWork,
...
const REVIEW_VERSION = 'passBSpatialExceptionReview/2';
...
async function renderPacket() {
  ...
  const workHtml = exceptionWorks.map((work, workIndex) => {
    const targetHtml = work.targets.map((target, targetIndex) => {
      ...
      const choices = [
        ...
        target.legacyPoint && ['use-legacy', `Use previous · ${fmtPoint(target.legacyPoint)}`],
    ...
    return `<section class="work" data-work="${esc(work.workId)}">
      ...
      <div class="review"><div class="work-progress"><span>Work ${workIndex + 1} of ${exceptionWorks.length}</span><label>Work note<textarea rows="2" data-work-note="${esc(work.workId)}" placeholder="Optional note about the whole work"></textarea></label></div>${targetHtml}
        <fieldset class="work-decision"><legend>Work status</legend><div class="choices"><button type="button" class="choice" data-work-choice="resolved" aria-pressed="false">Ready when exceptions resolved</button><button type="button" class="choice" data-work-choice="more-review" aria-pressed="false">Needs more review</button><button type="button" class="choice" data-work-choice="abstain" aria-pressed="false">No decision yet</button></div></fieldset>
      </div></section>`;
  }).join('');
  ...
  <main><section class="summary"><h2>${summary.exceptions} decisions need your attention; ${summary.targets - summary.exceptions} were resolved automatically</h2><p>Only held exceptions appear below. Distributed/global observations and divergent representative examples remain useful as unpinned notes; validated existing points stay pinned. Unique-point disagreements and uncertain or missing visible claims stop here.</p><div class="metrics"><span class="metric">${summary.presentations.pin || 0} pins</span><span class="metric">${summary.presentations.note || 0} notes</span><span class="metric">${summary.exceptions} holds</span><span class="metric">${summary.compoundNoteRoutes} compound → note</span></div></section>
  ...
  const ensureWork=id=>state.works[id]||(state.works[id]={decision:'abstain',note:'',targets:{}});const ensureTarget=(wid,key)=>ensureWork(wid).targets[key]||(ensureWork(wid).targets[key]={decision:'abstain',point:null,note:''});
  ...
  function choose(article,value,p=null){const key=article.dataset.key,wid=article.closest('.work').dataset.work,t=ensureTarget(wid,key);t.decision=value;t.point=p;article.querySelectorAll('[data-choice]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.choice===value)));article.querySelector('.placement-help').hidden=value!=='manual'||!!p;article.querySelector('.saved').textContent=value==='manual'&&!p?'Click the artwork to finish.':'Saved locally.';placing=value==='manual'&&!p?{wid,key,article}:null;save();draw(article.closest('.work'),key);}
