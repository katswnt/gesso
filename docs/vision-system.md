...
| VSD-047 | 2026-09-25 | **Cloud-credit B1–B3 lane.** Spend the owner's one-time Claude Code cloud credits (claim by 2026-10-07, expire 2026-11-04; cloud sessions only) on B1–B3 for works BEYOND the 30-day window, in the normal priority order (Easy first), while the Mac's subscription collector keeps the window. Same prompts, model, validation, provenance (`apiKeySource:none`, verified by a one-call cloud probe), confinement and fatal rules; the 00:00–08:30 hours rule does not apply because it protects the subscription, and a dollar cap on client-reported list-price spend replaces it (global $240, tracked across sessions in the private repo). Evidence travels only through the PRIVATE repo `katswnt/gesso-pass-b-evidence` (the game repo is public) and is imported locally only after per-file hash checks and full `inspectWork` re-verification of each work in a staging run dir; a local work always wins and nothing is overwritten. Cloud reservations and epochs are not imported. | Owner approved 2026-09-25 (private repo + hours exemption). Implemented with round-trip tests (33 checks, real evidence). Billing pool of cloud child calls still unconfirmed; the 5-work pilot checks it. No publication path |
| VSD-048 | 2026-09-25 | **B4 conformance fixes after the /5 canary held 5 of 10 works for rule-following, not content.** (1) A repeated `StructuredOutput` inside ONE call is accepted only when the CLI itself rejected every earlier emission, accepted the last, and the final `structured_output` is exactly that last emission (a CLI-internal resubmission, not a controller retry); any other repeat stays held. (2) For structured /3 deltas, a hotspot with an invalid `evidenceRef` or a `pinRef` that is not a pinned B1 candidate for the same evidence is SUPPRESSED with a recorded reason instead of rejecting the whole record; it is never replaced by a guessed pin, and any grounding or conflict targeting it becomes unresolved and widens to work scope (VSD-027 semantics). Archived /2 deltas keep the hard rejection, so the 44-set baseline is byte-identical. (3) The B4 prompt spells out the guide image-majority counts, the 500-character why cap and the hotspot anchor rule. Contracts: `passBValidationB4/2-structured-grounding`, canary `passBStructuredB4Canary/6`. | Implemented offline. Replaying all 22 preserved structured-B4 attempts: the /5 canary goes from 5/10 to 9/10 accepted (integrity only; zero eligible components); the one remaining hold (St. John guide majority) is the prompt-only class. The prompt change is unmeasured until the next canary. No model calls |
