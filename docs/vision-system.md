...
tool-less B4 `/3` attempts under a hard cap of ten durable pre-call reservations. Its pre-call manifest
...
`passBStructuredB4Canary/3`, at least one initialization event is required and **every** init must
report `apiKeySource:none`; a later clean init cannot mask an earlier wrong/missing source.
Model drift or any `*tool_use` execution block is fatal, including server tools and streamed blocks.
The command policy binds a six-minute process timeout with `SIGKILL`; captured timeout evidence is
...
| Pass B structured B4 canary | **Plan-first runner hardened and tested offline as `passBStructuredB4Canary/3` (VSD-040); no live calls yet.** The frozen ten-work selection and exact preserved B0–B3 bindings are unchanged. The runner writes its binding-verified execution manifest before calls and exclusively creates and flushes each pre-call reservation and syncs its directory entries; ten total reservations govern the cap and global contiguity across resumes. Reservation + verified transcript/result/meta evidence determines state independently of checkpoints: accepted/held/fatal/unknown-outcome are terminal, and only usage-limit may retry using a new slot. Any preserved fatal or unknown outcome stops calls before scheduling; reports retain all preserved work states, including later fatal rows. It verifies the canonical sealed-finding artifact/IDs and has no resolution/approval path. Results remain schema/scoping smoke only, with zero model-derived eligibility. Every init must report subscription provenance; all `*tool_use` execution blocks, including server/streamed events, fail fatally. A policy-bound six-minute hard timeout is terminal and never retried. The 79 offline canary checks pass; live execution still needs independent audit acceptance and explicit owner spend authorization |
