...
`passBStructuredB4Canary/4`, at least one initialization event is required and **every** init must
...
`*tool_use`, every server tool (even one named StructuredOutput), duplicate adapter emission, extra
init tool or model drift is fatal. Missing emission cannot be accepted; interruption/usage-limit
...
unchanged. `/4` creates a new identity and fresh ten-slot budget and refuses to resume earlier
contracts; it requires new owner authorization. Offline `/4` replay of St. John passes schema/scoping
...
failure is an A4 presupposition/aliasing challenge, not evidence of factual success. One historical
B4 transcript emitted the adapter twice after a wire-schema rejection; the requested exactly-once
rule intentionally rejects it too. Real transcript fixtures supplement the synthetic regressions.
...
| Pass B structured B4 fork | **Implemented (VSD-039); the owner-run VSD-040 smoke attempted two delta `/3` calls, with one completed output available for offline evaluation.** The model emits structured claim/observation bindings and component scopes only; deterministic hydration validates and translates them into stable final ids in `passBStructuredGrounding/1`. Dangling refs reject; suppressed/untranslatable targets widen the concern to work scope. Reconciliation policy `/2-structured-b4` consumes the exact scope while retaining model grounding as a non-authoritative proposal. Archived `/2` deltas and all 44 policy-`/1` baseline sets remain valid and were not regenerated. No release authority, approval, or production path was added |
| Pass B structured B4 canary | **Execution `/4` repaired offline after the owner-run `/3` smoke; 110 canary checks pass.** `b4s-016f64c8e0ca` remains terminal and byte-preserved at two reservations (La Gloire timeout held; St. John false tool-provenance fatal). New plan `b4s-18bc5fb62997` has its own ten durable pre-call reservations and no run directory or live calls. Every init must report subscription provenance and exactly `[StructuredOutput]`; exactly one `tool_use:StructuredOutput` output emission is permitted, and every other/duplicate/server tool event or extra init tool remains fatal. Timeout is 900 s, sized from preserved B4 durations with headroom. Real CLI 2.1.280 St. John output now derives accepted for schema/scoping only, with 21 review-required / zero eligible components; its animal presupposition remains a semantic failure fixture. Reservation/flush/contiguity, checkpoint-independent terminality, usage-limit-only retries, global fatal/unknown stop, all-work reporting and canonical-finding bindings are unchanged. No resolution/approval path exists. The frozen inputs, B1–B3 evidence and delta `/3` contract are unchanged. A new live smoke needs explicit owner authorization |
