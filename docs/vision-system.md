...
- A localization checker validates every same-image candidate independently. It does not
  ...
  checker—shown no earlier coordinates or reasoning—independently returns the same spatial
  scope and a point within 5 percentage points at confidence ≥0.75. Otherwise it requires
  human review. This selection is deterministic and never rewrites the observation.
- Spatial auto-resolution has three explicit trust classes: a validated existing candidate
  ...
  ambiguous, low-confidence, scope-disagreeing, and point-disagreeing results remain held.
...
| Pass B synthesis | **Compact editorial-delta B4 + deterministic hydration is implemented, tested, and run across the 50-work calibration.** VSD-027 separates editorial `ref` from spatial `pinRef`. VSD-029/030 add an offline spatial policy and candidate-validation checker: every same-image legacy/current point is judged independently; a valid current point is preserved as the no-churn tie-breaker and an uncertain candidate blocks relocation. Global/distributed observations route to notes; not-found/ambiguous observations hold. VSD-031 prevents any one-call new coordinate from resolving automatically: a separate fresh-context checker sees no earlier point/reasoning and must match scope and land within 5 points at confidence ≥0.75, otherwise the proposal holds. Re-resolving the accepted ten-work canary `b5c-f2020a1d8cca` under this policy yields 23 validated existing pins, 12 notes, and 9 holds, including 4 bounded confirmation targets. Confirmation canary `b5k-9e3a46a675ae` completed 4/4 strict-valid without retries and confirmed 0 automatically: 2 point-distance disagreements, 1 scope disagreement, and 1 low-confidence/scope mismatch were all held. Results remain calibration-only, no corpus-scale auto-policy is approved, and no Pass B output is connected to a production sink |
