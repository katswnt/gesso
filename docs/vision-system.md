...
### Content-correctness and reconciliation (VSD-034)
...
transitions are append-only and reopenable on new contradictory evidence.
...
catalog claims, never from raw B1 hypotheses. The reconciliation output is an immutable
report bound to the full B0–B4 + legacy evidence, not a mutation of any completion. The
claim/entity/component schema that makes these checks mechanical is specified but built only
as an adversarial canary shows models can emit it reliably and that the exception economics
are affordable; auto-policy stays gated on that measurement, and VSD-011 human sampling
continues regardless. See the decision log (VSD-034).
...
| Pass B content-correctness / reconciliation | VSD-034 authorized scope IMPLEMENTED OFFLINE + gated (items 1–3), item 4 in progress. Item 1: immutable ancestry-bound `content-blocked` findings (La Gloire `wikidata:Q16467705`, St. John Chrysostom `wikidata:Q1211814`) sealed in the tracked `data/vision-content-blocked.json` and enforced in the guarded Pass B approval path — mandatory, cwd-independent, fail-closed; blocked works are intentionally uncleared (no resolution wired). Item 2: quarantined experimental entity-emission contract `contentVisionEntityGraph/1` + evaluator (invalid emissions score zero; global matching; exhaustive-aware; alias precision/recall). Item 3: controller `passBEntityController/2` (explicit type-incompatibility; model-declared relations annotate, never suppress; max-region-IoU) + seeded sealed canary fixtures (`data/vision-entity-canary-fixtures.json`) whose GOLD scenes contain only real entities (zero gold aliases; holdout is authoritative-source-seeded/precision-only, recall undefined until owner pixel-labeling). Item 4: offline validation + a bounded model canary — NOT yet run (no model calls). The reconciliation report, curated index, full claim graph, and any auto-policy remain designed-not-built and gated on canary metrics. Nothing reaches the existing guarded Pass B approval writer (which has no approved records and does not publish rich descriptors); no production sink |
