...
| Pass B synthesis | **Compact editorial-delta B4 + deterministic hydration is implemented, tested, and run across the 50-work calibration.** VSD-027 separates editorial `ref` from spatial `pinRef`. VSD-029 adds an offline spatial policy that preserves same-image legacy coordinates as candidates; routes duplicate, broad, distributed, and uncertain observations without deleting their content; and prepares a bounded localization-only canary whose prompt receives no research, player copy, or owner answers. The owner export measures 210 observations: 73 deterministic placements, 122 localization exceptions, 11 note routes, and 4 merges. The canary is plan-only and has not run. No Pass B output is connected to a production sink |
