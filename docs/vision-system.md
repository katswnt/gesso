...
`passBStructuredGrounding/1` sidecar contains proposals only: no authority, effective state,
resolution, approval, or eligibility field exists in the model wire schema. Reconciliation policy
`passBReconciliationPolicy/2-structured-b4` consumes the translated scope but continues to hold
...
tool-less B4 `/3` attempts under a hard ten-attempt total cap. Its pre-call manifest binds those source bytes,
effective B4 prompts, command policy, model, schema/validation versions, and canonical blocked-finding
ids; its ten-attempt cap is total across resumes, with zero validation retries (a preserved usage-limit
rejection may retry after reset, but still consumes the cap). Wrong/missing subscription provenance, model drift,
or any B4 tool use aborts. Shape/hydration/leak/reconciliation failures are held without retries, and
La Gloire/St. John remain sealed regardless of fresh output. `accepted` means only execution,
...
| Pass B structured B4 fork | **Implemented and tested offline (VSD-039); no live `/3` call yet.** The model emits structured claim/observation bindings and component scopes only; deterministic hydration validates and translates them into stable final ids in `passBStructuredGrounding/1`. Dangling refs reject; suppressed/untranslatable targets widen the concern to work scope. Reconciliation policy `/2-structured-b4` consumes the exact scope while retaining model grounding as a non-authoritative proposal. Archived `/2` deltas and all 44 policy-`/1` baseline sets remain valid and were not regenerated. No release authority, approval, or production path was added |
| Pass B structured B4 canary | **Plan-first runner implemented and tested offline (VSD-040); no live calls yet.** A frozen ten-work selection includes both canonical failures, prior assert-vs-hedge/iconography regressions, multi-figure works, a changed-image regression, and controls. The runner reopens and binds the preserved source completion/raw/transcript bytes, writes its write-once-by-tool, binding-verified execution manifest before a call, enforces ten total attempts across resumes with zero validation retries, re-derives checkpoints from transcripts, aborts on subscription/model/tool-policy drift, and labels results as schema/scoping smoke only. It cannot approve, resolve, publish, or create model-derived eligibility. Live execution remains owner-gated pending independent audit |
