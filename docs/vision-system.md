...
| Current approval | The quarantined full-cohort packet captures non-authoritative work decisions, per-work notes, and per-hotspot keep/move/drop/keep-as-note choices (including click-to-place coordinates) in browser-local state and exports a run/evidence-bound JSON handoff (VSD-028). A separate one-work, hash-bound guarded Pass B approval/apply path supports explicit field-level owner approval and labeled owner edits; no calibration record is yet approved. Auto-policy and component-ledger publication remain unimplemented |
