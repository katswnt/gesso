...
| Pass B synthesis | **Compact editorial-delta B4 + deterministic hydration is implemented, tested, and run across the 50-work calibration.** VSD-027 separates editorial `ref` from spatial `pinRef`. VSD-029/030 add an offline spatial policy and candidate-validation checker: every same-image legacy/current point is judged independently; a valid current point is preserved as the no-churn tie-breaker and an uncertain candidate blocks relocation. Global/distributed observations route to notes; not-found/ambiguous observations hold. VSD-031 prevents any one-call new coordinate from resolving automatically through a separate fresh-context checker. VSD-032 incorporates the canary distinction between unique and representative/distributed targets: distant unique points hold, while independently divergent representative examples and a confirmation-classified distributed/global target become notes. The offline resolver re-verifies all source/transcript/image/model/subscription bindings and renders only holds. For `b5c-f2020a1d8cca` + `b5k-9e3a46a675ae`, derived run `b5r-3a5ea5ed593f` records 23 validated pins, 14 notes, and 7 holds; the owner packet contains only those 7 exceptions across 6 works. Results remain calibration-only, no corpus-scale auto-policy is approved, and no Pass B output is connected to a production sink |
