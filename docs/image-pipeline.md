...
- **Phase 3:** the efficiency refactor — collapse the ~6 whole-pool Wikidata sweeps into one cached `wd-entities.json` pull, and have the image audits read dimensions from the fingerprint baseline (re-fetch only drifted URLs). Once the suite is near-instant it can run automatically inside every promote — and a stored-native-width resolution gate (aspect-ratio-aware) can move into check-pool.
