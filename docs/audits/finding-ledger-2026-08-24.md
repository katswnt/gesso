...
authorization is now **installed in observe mode** — **LIVE in production** as of 2026-08-26 (deployed `6c9288d`;
`CAP_MODE=observe` + `CRON_SECRET` set in Vercel Production; live smoke confirmed register/observe/wrong-cap/
malformed/missing-cap-legacy behavior). New `server/api/{http,supabaseAdmin,auth,device-ownership}.js` (shared origin/body, service-role + user-JWT boundary,
...
foreign-device-deletion exploit) + deletes `saves` + the account's `devices` rows; `event` untouched. Client
(`index.html`): `capability()` + Web-Locks-serialized `ensureDeviceRegistered()` (awaited before every gated call;
always sends the cap — never a cap-less downgrade; rotation-safe builder; new-identity fallback), and a visible
device-conflict banner. `CAP_MODE` precedence: explicit observe overrides the `2026-09-24` deadline, invalid fails
closed. State machine: only a **missing** cap is grandfathered, and only in observe; wrong/malformed/unregistered/
revoked are rejected in both modes. Tests: `tests/api-device-ownership.test.mjs` (47 assertions — hostile matrix,
profile/score authority from `devices.user_id`, header contract, CAP_MODE deadline, deletion contamination) +
`tests/client-capability.test.mjs` (11 — registration-failure/retry/409-rotation/banner/builder inventory), both in
`test:ci`. **SEC-1/SEC-2/SEC-6 → authorization installed, observe mode; the missing-cap legacy exposure remains until
3C enforcement.** SEC-3 foreign-device-deletion **fixed** (3B). Deferred to **PR 4**: full transactional erasure,
...
functions/triggers; migration objects intact. **SEC-4 remains partial** — legacy-gap metric reported **7** at
measurement time, but that count was inflated by the verifier's own transient fixtures; the steady-state figure is
**4** (see the 2026-08-27 reconciliation note). SEC-4 partial while > 0. **SEC-4
...
**Implementation log — PR 4 Part 4B (API + client erasure wiring), 2026-08-25 (built) → LIVE in production 2026-08-26 (deployed `6c9288d`):**
...
**Production rollout — 3B + 4A + 4B went LIVE (observe), 2026-08-26.** `CAP_MODE=observe` + `CRON_SECRET` set in
Vercel Production; `main` fast-forwarded `50f437c → 6c9288d` and pushed; deploy `artefactum-5bzyjei71` **Ready** and
serving `gesso.katswint.com` (HTTP 200). Live smoke (all green): homepage + a Serverless Function load; fresh
anonymous device registers; missing-cap legacy path works (observe); valid-cap save/profile flows work; **wrong cap→
401 / malformed→400 even in observe**; event telemetry cap-independent; purge endpoint unauth→401, wrong-method→405
(CRON_SECRET never sent); a **disposable-account deletion through the deployed handler** erased all owned rows +
tombstone + auth user (verified in DB). Cross-account claim HTTP smoke was inconclusive (disposable two-user
provisioning collapsed to one user under GoTrue signup rate-limiting → same-user re-claim); authority verified held
in DB and cross-account 409 rejection is proven by the test suite + `db:verify` (the same two-user conflict was
proven against production during 3A DB verification). All disposable smoke artifacts cleaned (0 residual).

**Deployment-limit incident (2026-08-26) — RESOLVED.** The first push (`6032770`, deploy `artefactum-fvmtv74qz`)
built cleanly but **errored at "Deploying outputs"**: every non-underscored file under `api/` (recursively) is a
deployable Vercel function, so the 4 helpers in `api/lib/` counted alongside the 12 endpoints = **16 > Hobby's
12-function cap** (`_supabase.js` is underscore-ignored; the once-daily cron is within Hobby limits). Production was
never affected — the prior good deploy stayed live. Fixed in **`6c9288d`** by moving the 4 helpers to `server/api/`
(outside the functions dir; api/ = exactly 12 endpoints), pure-mechanical import rewrites, and a permanent gate
...
**Guarded writes — Stage B LIVE (observe), 2026-08-27 (deployed `5763503`, `artefactum-5lrpkvrd0`).** The verified-cap
paths in `saves`/`score`/`profile`/`claim`/`sync` now call the guarded RPCs (serialized against erasure);
missing-cap legacy paths retained + logged (the accepted observe-mode exposure until enforce). Result envelopes are
fully validated (success shape per function, `ok`↔`result` consistency, contradictory/`ok:false`-without-error
rejected as 502); verified paths never fall back to a raw write; `sync` does one checked 4-column `user_state` read
(0/1 rows, all columns) → one JS merge → one guarded write. Live smoke: register/save/score/profile/claim/sync
verified + legacy compat + wrong→401/malformed→400 + purge 401 + event 200; a disposable-account claim→sync→delete
through the deployed handlers erased all owned rows + auth user; zero residue after cleanup. No client change,
`CAP_MODE=observe`, api/ 12/12. **SEC-4 residual (1) is now closed for the verified-cap path.**

**SEC-4 legacy-tail reconciliation (read-only, 2026-08-27).** Steady-state legacy count is **4 profile-only device
histories across 2 live accounts**, NOT 7 — the earlier 7 included **three transient verifier fixtures** (measured
mid-`db:verify` run). Capability adoption is nascent: **1 observed returning device since the cap client shipped
(2026-08-26), and it has registered (1 registered)** — this is one data point, not a meaningful "100% adoption
rate," and no enforcement decision follows from one day of data. Of the 4 residual histories, **three belong to
account `dfba049f`, which is already using a new capability device** (`bcc9b98d`, registered 2026-08-26), so they are
superseded prior-device history; **one belongs to a dormant account** (`be9df431`, last active 2026-06-21, no cap).
**SEC-4 remains partial** because account erasure resolves devices only from `devices.user_id`, so it cannot safely
reach these 4 histories (no `devices` row), and the server cannot mint capabilities for them (it holds only SHA-256
hashes, never the client's raw secret). Plan: let active self-healing run a few weeks, then remeasure and design the
legacy-tail PR around the residual population. **Invariant the future design must preserve: a contaminated
`profiles.user_id` can NEVER authorize deletion of data attached to another account's authoritative `devices` row.**
`CAP_MODE` remains `observe`; no enforcement flip.
