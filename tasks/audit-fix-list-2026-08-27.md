...
- [x] **G-02 — FIXED & LIVE 2026-08-27 (deny-by-default public output dir).** `vercel.json` used `outputDirectory: "."`, making tracked files under `/docs`, `/tasks`, `/db`, `/tests`, `/server`, and `/scripts` fetchable (verified reconnaissance-only: no secrets/participant data, RLS held, `devices` revoked). Fix (chose the long-term option): a canonical `npm run build` assembles a generated, gitignored `public/` containing only the browser assets the app loads; `outputDirectory: "public"`; SPA rewrites narrowed to the exact route set so everything else returns a real 404; `api/**`+`server/api/**` stay at root as function source. New offline gate `scripts/check-public-output.mjs` + `tests/routes.test.mjs` (fail-closed) in `test`/`test:ci`. Codex-reviewed (5 findings fixed in `6bc907c`). **Live:** prod SHA `e0dfcb6`, deployment `artefactum-e4uw655r2`, production matrix **47/47** (every private tree / source / meta path now returns a real 404; `/api/score.js` is a function 405 with no source). Non-blocking follow-up: reject nested `data/notes` subdirs + assert referenced assets exist in the assembled output.
